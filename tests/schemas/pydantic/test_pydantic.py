...
def test_pydantic_record_self_one_to_many_relationship(
    ...
    class User(AvroBaseModel):
        ...
        teamates: typing.List["User"] = None
...
def test_pydantic_record_self_one_to_many_map_relationship(
    ...
    class User(AvroBaseModel):
        ...
        teamates: typing.Dict[str, "User"] = None
...
def test_validate():
    ...
    with pytest.raises(ValidationError):
        User.model_validate({"name": "a name"})
...
def test_json_schema(AvroBaseModelV2):
    assert AvroBaseModelV2.json_schema()


def test_to_dict(AvroBaseModelV2):
    instance = AvroBaseModelV2(first_union="hi!", logical_union=uuid.uuid4())
    assert instance.to_dict() == instance.model_dump()
...
def test_to_json(AvroBaseModelV2):
    instance = AvroBaseModelV2(first_union="hi!", logical_union=uuid.uuid4())
    assert (
        instance.to_json(
            separators=(
...
def test_to_json_logical_types():
    ...
    to_json = logical_types.model_dump_json()

    assert (
        logical_types.to_json(
            separators=(
                ",",
                ":",
            )
...
def test_exclude_field_with_default() -> None:
    class Message(AvroBaseModel):
        internal_field: str = Field(exclude=True, default="internal")
        public_field: str
