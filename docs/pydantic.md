...
Serializing your model into a `dict` (which will be serialized to Avro) does not always use the built-in Pydantic serialization functionality though. Any [Pydantic serialization](https://docs.pydantic.dev/latest/concepts/serialization/) logic you have on your model may not be used when calling `AvroBaseModel.serialize` or `AvroBaseModel.asdict`. Instead, you have to map any custom types to a `CustomAvroEncoder` in the `Meta.custom_encoders` field on your model to specify the supported Avro-serializable python type and a function to convert the field type into the supported type.
...
You can use a [Pydantic custom schema](https://docs.pydantic.dev/latest/concepts/types/#customizing-validation-with-__get_pydantic_core_schema__) in combination with a `CustomAvroEncoder` to work with a custom type.
...
class MyModel(AvroBaseModel):
    ...
    class Meta:
        custom_encoders = {
            CustomClass: CustomAvroEncoder(return_type=str, to_avro=lambda x: x.value)
        }
...
class MyModel(AvroBaseModel):
    ...
    class Meta:
        custom_encoders = {
            timedelta: CustomAvroEncoder(return_type=float, to_avro=lambda x: x.total_seconds())
        }
...
### Deprecated Custom Types with `json_encoders` and `__get_pydantic_core_schema__`

The custom serialization type and function can alternatively be defined in the Pydantic model config [`json_encoders`](https://docs.pydantic.dev/latest/api/config/#pydantic.config.ConfigDict.json_encoders) instead of a `CustomAvroEncoder` annotation, but `json_encoders` is deprecated in Pydantic. You should not use this strategy and the `CustomAvroEncoder` strategy at the same time.
