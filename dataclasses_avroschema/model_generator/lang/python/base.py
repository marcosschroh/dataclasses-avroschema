...
class FieldRepresentation:
    ...
    def __post_init__(self) -> None:
        ...
        # if self.is_root and self.name in self.type_hint:
        #     # if the name is equal to the last child type_hint
        #     # then it means the there is a clashe
        #     field_aliases = self.metadata.get("aliases", [])
        #     field_aliases.append(self.name)
        #     self.metadata["aliases"] = field_aliases
...
class BaseGenerator:
    ...
    model_names_generated: typing.Set[str] = field(default_factory=set)
    ...
    def render_class(self, *, schema: JsonDict) -> str:
        ...
        # add the class name generated to the set to compare later with all the fields
        self.model_names_generated.add(model_name)
        ...
        fields_representation: typing.List[FieldRepresentation] = [
            self.render_field(field=field, model_name=model_name, is_root=True) for field in record_fields
        ]
    ...
    def render_field(self, field: JsonDict, model_name: str, is_root: bool = False) -> FieldRepresentation:
        """
        ...
        if self.is_logical_type(field=field):
            ...
            type_hint, children = self.parse_array(field=field, model_name=model_name)
        elif type == field_utils.MAP:
            type_hint, children = self.parse_map(field=field, model_name=model_name)
    ...
    def parse_logical_type(self, *, field: JsonDict) -> str:
        ...
        if type_from_metadata is not None:
            ...
            type_hint = self.get_language_type(type=field["type"])
        else:
    ...
    def parse_array(self, field: JsonDict, model_name: str) -> typing.Tuple[str, typing.List[FieldRepresentation]]:
        """
        Parse an Avro array
        ...
        field_representation = self.render_field(field=avro_type, model_name=model_name)
        type_hint = templates.list_template.safe_substitute(type=field_representation.type_hint)

        return type_hint, [field_representation]

    def parse_map(self, field: JsonDict, model_name: str) -> typing.Tuple[str, typing.List[FieldRepresentation]]:
        """
        Parse an Avro map
